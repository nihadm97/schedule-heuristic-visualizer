...
function cost(x) { // temporaly cost function
	let sum = 0
...
/*
function cost(x) { // reference error in checkProfessorBreakAndNumOfLessons on Line 288, needs to be resolved
	let sum = 0
	for (let i = 0; i < x.length; i++) {
		if(checkProfessorBreakAndNumOfLessons(x)){
      sum += 5000;
    }
	}
	return sum
...
function switchTimes(arrayTimes, arraySolution){ // switch old timeslots with new timeslots
...
function batAlgorithm(costFunc, saveRate = 100, maxGen = 1000, popSize = 50, solution = tempSolution, maxLoudness = 2, 
                      maxPulseRate = 1, fMin = 0, fMax = 10, lowerBound = 0, upperBound = time.length) {
		...
		for (let gen = 1; gen <= maxGen; gen++) {
			...
			for (let i = 0; i < popSize; i++) {
				...
				if(getRdn(0,1) > pulseRate[i]) {
					for (let j = 0; j < solution.length; j++) {
						// getRdn(-1, 1) = epsilon
						newPosition[i][j] = Math.round(bestBat[j] + getRdn(-1,1) * average(loudness)); // random walk (5)
						newPosition[i][j] = checkBound(newPosition[i][j], lowerBound, upperBound);
					}
				}
...
const result = batAlgorithm(cost, 150, 10000, 100, tempSolution, 2, 1, -10, 10, 0, time.length);
console.log(result);
